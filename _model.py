    ...
    def action(self, project, command, collection, **kwargs):
        ...
        if command.lower() == "train":
            try:
                ...
                if checkpoint:
                    model = YOLO(f"checkpoints/{checkpoint}")
                else:
                ...
                result = model.train(data=data, imgsz=imgsz, epochs=epochs, project=f"./{project}")
            ...
            try:
                ...
                if data != {}:
                    ...
                    return {"message": "predict completed successfully", "result": {"boxes":result[0].boxes.xyxy.tolist(),
    ...
    def model(self, project, **kwargs):
        ...
        with gr.Blocks(css=css) as demo:
            ...
            def get_checkpoint_list(project):
                ...
                import os
                checkpoint_list = [i for i in os.listdir("models") if i.endswith(".pt")]
                checkpoint_list = [f"<a href='./checkpoints/{i}' download>{i}</a>" for i in checkpoint_list]
                if os.path.exists(f"{project}"):
                    for folder in os.listdir(f"{project}"):
                        if "train" in folder:
                            project_checkpoint_list = [i for i in os.listdir(f"{project}/{folder}/weights") if i.endswith(".pt")]
                            project_checkpoint_list = [f"<a href='./{project}/{folder}/weights/{i}' download>{folder}-{i}</a>" for i in project_checkpoint_list]
    ...
    def model_trial(self, project, **kwargs):
        ...
        with gr.Blocks(css=css) as demo:
            ...
            def download_btn(evt: gr.SelectData):
                ...
                return f'<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css"><a href="/datasets/{evt.value}" style="font-size:50px"> <i class="fa fa-download"></i> Download this dataset</a>'
